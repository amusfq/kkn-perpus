...
import PublisherType, {
  PublisherPagination,
} from "../../../models/PublisherType";
...
export default function Publisher({}: Props) {
  ...
  const [data, setData] = useState<PublisherPagination>();
  ...
  const updateStatus = (row: PublisherType) => () => {
    Swal.fire({
      title: `Anda yakin ingin menghapus penerbut ${row.name}?`,
      icon: "question",
      ...
      if (response.isConfirmed) {
        setIsLoading(true);
        Axios.delete(`/publisher/${row.id}`)
          .then(async () => {
            toast.success(`Berhasil menghapus penerbit ${row.name}`, {
              theme: "colored",
            });
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col md:flex-row items-center justify-between space-y-2 md:space-y-0">
        ...
        <Button primary to="/admin/publisher/create">
          Tambah
        </Button>
      </div>
      {
