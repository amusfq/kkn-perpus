...
import ShelfType, { ShelfPagination } from "../../../models/ShelfType";
...
export default function Shelf({}: Props) {
  ...
  const [data, setData] = useState<ShelfPagination>();
  const navigate = useNavigate();

  const { register, handleSubmit, watch } = useForm<Values>();
  const watchQ = watch('q')

  const columns: TableColumn<ShelfType>[] = [
    {
      name: "Aksi",
      cell: (row) => (
        <div className="flex flex-col space-y-1">
          <Link
            to={`/admin/shelf/${row.id}`}
  ...
  const updateStatus = (row: ShelfType) => () => {
    Swal.fire({
      title: `Anda yakin ingin menghapus rak buku ${row.name} - ${row.location}?`,
      icon: "question",
      ...
      if (response.isConfirmed) {
        ...
        Axios.delete(`/shelf/${row.id}`)
          .then(async () => {
            toast.success(
              `Berhasil menghapus rak buku ${row.name} - ${row.location}`,
              {
                theme: "colored",
              }
            );
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col md:flex-row items-center justify-between space-y-2 md:space-y-0">
        ...
        <Button primary to="/admin/shelf/create">
          Tambah
        </Button>
      </div>
      {
